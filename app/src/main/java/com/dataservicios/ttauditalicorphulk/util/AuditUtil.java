...
import com.dataservicios.ttauditalicorphulk.Model.Cuota;
import com.dataservicios.ttauditalicorphulk.Model.Media;
import com.dataservicios.ttauditalicorphulk.Model.Pdv;
import com.dataservicios.ttauditalicorphulk.Model.PhoneDetail;
import com.dataservicios.ttauditalicorphulk.Model.PollDetail;
import com.dataservicios.ttauditalicorphulk.Model.User;
...
//
    ...
    public static Cuota getCuota(int store_id){
        int success ;
        ...
        try {
            ...
            JSONParserX jsonParser = new JSONParserX();
            // getting product details by making HTTP request
            JSONObject json = jsonParser.makeHttpRequest(GlobalConstant.dominio + "/JsonGetCuotaVenta" ,"POST", params);
            // check your log for json response
            Log.d("Login attempt", json.toString());
            // json success, tag que retorna el json
            if (json == null) {
                ...
                if (success == 1) {
                    JSONArray ObjJson;
                    ObjJson = json.getJSONArray("cuota_venta");
                    // looping through All Products
                    if(ObjJson.length() > 0) {
                        ...
                        for (int i = 0; i < ObjJson.length(); i++) {
                            ...
                            cuota.setStore_id(Integer.valueOf(obj.getString("mercado")));
                            cuota.setCuota(obj.getString("cuota"));
